...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure MenuColorInversionClick(Sender: TObject);
    procedure MenuFloodFillV4Click(Sender: TObject);
    procedure MenuFloodFillV8Click(Sender: TObject);
...
procedure FloodFill4(bmp: TBitmap; x, y: Integer; FillColor: TColor);
var
  ...
  top: Integer;
begin
  oldColor := bmp.Canvas.Pixels[x, y];
  ...
  begin
    p := stack[top];
    Dec(top);
    if (p.X < 0) or (p.X >= bmp.Width) or (p.Y < 0) or (p.Y >= bmp.Height) then Continue;
    if bmp.Canvas.Pixels[p.X, p.Y] <> oldColor then Continue;

    bmp.Canvas.Pixels[p.X, p.Y] := FillColor;

    Inc(top); SetLength(stack, top+1); stack[top] := Point(p.X+1, p.Y);
    Inc(top); SetLength(stack, top+1); stack[top] := Point(p.X-1, p.Y);
    Inc(top); SetLength(stack, top+1); stack[top] := Point(p.X, p.Y+1);
    Inc(top); SetLength(stack, top+1); stack[top] := Point(p.X, p.Y-1);
  end;
end;
...
begin
  ...
  begin
    draw := true;
    Image1.Canvas.Pixels[X,Y] := PolygonColor;
  end
  else if operation = 10 then
  begin
    bmp := TBitmap.Create;
    try
      bmp.Assign(Image1.Picture.Bitmap); // copy bitmap
      FloodFill4(bmp, X, Y, PolygonColor); // fill bmp
      Image1.Picture.Bitmap.Assign(bmp);   // updates Image1
    finally
      bmp.Free;
    end;
    Image1.Refresh;
...
procedure TForm1.MenuColorInversionClick(Sender: TObject);
begin
  operation := 9; //fill polygon by color inversion
end;

procedure TForm1.MenuFloodFillV4Click(Sender: TObject);
begin
  operation := 10; //fill polygon by flood-fill technique (neighborhood-4)
end;

procedure TForm1.MenuFloodFillV8Click(Sender: TObject);
begin
  operation := 11; //fill polygon by flood-fill technique (neighborhood-8)
end;
